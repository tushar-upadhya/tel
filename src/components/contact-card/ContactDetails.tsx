...
const ContactDetails: React.FC<ContactDetailsProps> = ({ selectedId }) => {
  ...
  if (!contactDetails || contactDetails.length === 0) return <p>No contacts available</p>;
  ...
  return (
    <>
      ...
      <Accordion type="single" collapsible className="mt-2 sm:mt-4">
        <FilteredContactList contacts={contactDetails} searchQuery={debouncedSearchQuery} />
      </Accordion>
    </>
  );
};
